				...
				<Card>
					...
					<CardContent>
						...
						<p className="text-xs text-muted-foreground">
							{dashboard?.completedRunsToday ?? 0} completed, {dashboard?.failedRunsToday ?? 0}{" "}
							failed
						</p>
					</CardContent>
				</Card>
					...
					<CardContent>
						<div className="text-3xl font-bold">
							{dashboard?.averageLatencyMs
								? `${(dashboard.averageLatencyMs / 1000).toFixed(1)}s`
								: "--"}
						</div>
					...
					<Card className="transition-colors hover:bg-muted/50">
						...
						<CardContent>
							<p className="text-sm text-muted-foreground">
								Set up tool schemas and execution policies
							</p>
						</CardContent>
					</Card>
				</Link>
				...
				{dashboard?.failedRunsToday ? (
					<Link to="/runs">
						<Card className="transition-colors hover:bg-muted/50 border-destructive/50">
							...
							<CardContent>
								<p className="text-sm text-muted-foreground">
									{dashboard.failedRunsToday} runs failed today
								</p>
							</CardContent>
						</Card>
					</Link>
